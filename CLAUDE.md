...
```
Env vars: `CROWSYNC_PORT`, `CROWSYNC_DB_PATH`, `CROWSYNC_STORAGE_ROOT` (defaults: `8001`, `./crowsync.db`, `./storage`), `CROWSYNC_ADMIN_TOKEN` (empty by default — required to register members after the first one, see Auth model), and `CROWSYNC_OPEN_REGISTRATION` (`1`/`true` = trusted-LAN mode: anyone registers/recovers by name, no token — for firewalled networks only, see Auth model).
...
`POST /members` is gated by `require_admin_or_bootstrap`: the **first** member registers freely (bootstrap mode); afterwards it must carry `X-Admin-Token` matching `CROWSYNC_ADMIN_TOKEN` (and is rate-limited per IP via `_rate_limit_members`). **Open LAN mode** (`CROWSYNC_OPEN_REGISTRATION=1`) bypasses the token entirely so any networked client can register (or re-register to recover a lost key) with just a name — meant for trusted/firewalled teams, never a public server. `/health` echoes `open_registration` (true in open or bootstrap mode) so `App.tsx` hides the admin-token field and shows an "Open LAN mode" hint. Destructive ops still require the token regardless. Re-posting an existing name (with a valid token) **issues a fresh key** via `storage.reset_member_key` — the original can't be returned since only the hash is stored, so recovery invalidates the old machine's key. The admin token is a master key to every account. The setup UI (`App.tsx:handleSetup`) sends the admin token and persists it as `crowsync_admin_token`.

**Server-wide destructive endpoints** (`DELETE /members/{id}`, `PUT /settings`) require `X-Admin-Token` via the strict `require_admin` dependency (no bootstrap). `client.ts:headers()` attaches `crowsync_admin_token` from localStorage to every request when present (harmless on non-admin routes). The env admin token is also a **super-admin** for per-project access (see below).

### Project membership & roles
Projects are **membership-scoped**: a member sees and acts on **only the projects they belong to**. Membership + role live in `project_members(project_id, member_id, role)` where `role ∈ {'admin','member'}` (the only module touching it is `storage.py`). The **creator** of a project becomes its first `admin` (`create_project(..., creator_id)`); anyone may create a project, so a new member always has a way in.
- Two FastAPI dependencies gate project-scoped routes (`main.py`): **`require_project_member`** (on ~all `/projects/{id}/…` endpoints — files, compare, lock, activity, stats, …; 403 if not a member) and **`require_project_admin`** (on `PUT/DELETE /projects/{id}` + the membership endpoints). `_project_role()` resolves the caller's effective role; the **env `CROWSYNC_ADMIN_TOKEN` passes both as a super-admin** (break-glass, even without a membership row). `GET /projects` returns only the caller's projects (each with their `role`); the super-admin token returns all. The WebSocket endpoint applies the same membership check after its auth frame.
- Membership endpoints: `GET /projects/{id}/members`, `POST /projects/{id}/members {member_id, role}`, `PUT /projects/{id}/members/{member_id} {role}`, `DELETE /projects/{id}/members/{member_id}` — all `require_project_admin`. A **last-admin guard** (409) blocks demoting/removing the only remaining admin. Frontend: `client.list/add/update/removeProjectMember` → `ProjectMembersDialog.tsx` (header `MEMBERS` button; admin-only controls keyed off `project.role`).
- **Migration**: `init_db` backfills membership once for pre-upgrade DBs (every active member → `member` of every project, lowest-id member → `admin`) so nobody loses access on upgrade.
...
### Project stats
`GET /projects/{id}/stats` (`main.py` → `storage.project_stats`) returns a read-only, aggregated snapshot for the **STATS** overlay: storage usage (current files + full version-history blob bytes), active locks grouped by holder, top contributors (upload activity per member), file-type breakdown by extension, and a daily activity heatmap (last 12 weeks). Pure `SELECT … GROUP BY` over existing tables — no new schema. Visible to any member (like activity). Frontend: `client.getStats` → `StatsDialog.tsx` (modal opened from the header `STATS` button in `SyncPage`); the heatmap is a CSS-grid (no chart lib).
